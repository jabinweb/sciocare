...
export default function BatchDripConfigPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchConfig = useCallback(async () => {
    try {
      ...
      setSavedSnapshot(
        JSON.stringify({
          isDripEnabled: data.isDripEnabled,
          dripStartDate: data.dripStartDate,
          subjects: data.subjects.map((s: SubjectConfig) => ({
            id: s.id,
  ...
  return (
    <div className="p-6 md:p-10 max-w-5xl mx-auto space-y-8">
      ...
      {/* Schedule warnings */}
      {config.scheduleWarnings && config.scheduleWarnings.length > 0 && (
        <div className="space-y-2">
          {config.scheduleWarnings.map((warning) => (
            <div
              key={warning}
              className="flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4 text-amber-900"
            >
              <AlertCircle className="mt-0.5 h-5 w-5 shrink-0 text-amber-600" />
              <p className="text-sm font-bold leading-snug">{warning}</p>
            </div>
          ))}
        </div>
      )}

      {hasUnsavedChanges && config.isDripEnabled && (
        <div className="flex items-start gap-3 rounded-xl border border-orange-200 bg-orange-50 p-4 text-orange-900">
          <AlertCircle className="mt-0.5 h-5 w-5 shrink-0 text-orange-600" />
          <p className="text-sm font-bold leading-snug">
            You have unsaved changes. The schedule shown below is not live for students until you click
            &quot;Save Schedule&quot;.
          </p>
        </div>
      )}
                  ...
                  <div className="flex items-center gap-4 flex-1 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                        Unit {index + 1}
                        {config.isDripEnabled && subject.isPersisted === false && (

...
export default function BatchesPage() {
  ...
  const toggleBatchActive = async (batch: Batch) => {
    const nextActive = batch.isActive === false;
    setTogglingBatchId(batch.id);
    try {
      const res = await fetch('/api/admin/batches', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: batch.id, isActive: nextActive }),
      });
      if (res.ok) {
        const updated = await res.json();
        setBatches((prev) =>
          prev.map((b) => (b.id === batch.id ? { ...b, isActive: updated.isActive } : b))
        );
      }
    } catch (error) {
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {batches.map((batch) => {
              ...
              return (
              <Card key={batch.id} className={!isActive ? 'opacity-75 border-dashed' : undefined}>
                <CardHeader>
                  <div className="flex justify-between items-start">
                    <div className="flex flex-col gap-1">
                      <CardTitle className="text-xl font-bold flex items-center gap-2 flex-wrap">
                        {batch.name}
                        {!isActive && (
                          <Badge variant="secondary" className="bg-slate-100 text-slate-600 border-slate-200 font-bold px-1.5 py-0 text-[10px] uppercase">
                            Inactive
                          </Badge>
                        )}
                        {batch.isDripEnabled && (
                        ...
                        {batch.isDripEnabled && (batch._count.dripConfigs ?? 0) === 0 && (
                          <Badge variant="secondary" className="bg-amber-50 text-amber-800 border-amber-200 font-bold px-1.5 py-0 text-[10px] uppercase">
                  ...
                  <div className="space-y-4">
                    ...
                    {canManage && (
                      <div className="flex items-center justify-between rounded-lg border bg-slate-50 px-3 py-2">
                        <div>
                          <p className="text-sm font-medium">Batch active</p>
                          <p className="text-xs text-muted-foreground">
                            {isActive
                              ? 'Students in this batch have access'
                              : 'Students in this batch are paused'}
                          </p>
                        </div>
                        <Switch
                          checked={isActive}
                          disabled={togglingBatchId === batch.id}
                          onCheckedChange={() => toggleBatchActive(batch)}
                    ...
                    {batch.isDripEnabled && (batch._count.dripConfigs ?? 0) === 0 && (
                      <div className="flex items-start gap-2 text-sm text-amber-700 mt-1">
                        <AlertCircle className="h-4 w-4 shrink-0 mt-0.5" />
                        <span>Drip enabled but schedule not saved — students still have full access.</span>
                      </div>
                    )}

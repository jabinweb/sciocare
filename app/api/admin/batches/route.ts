...
import { isAdminTeacherOrModerator, isAdminOrModerator } from '@/lib/auth-utils';
import { NextResponse } from 'next/server';

export async function GET(request: Request) {
  const [authOk, authVal] = await isAdminTeacherOrModerator();
  if (!authOk) return authVal;
  const session = { user: authVal };
  ...
  try {
    ...
    const batches = await prisma.batch.findMany({
      where: {
        ...
        ...(teacherId
          ? {
              OR: [{ teacherId }, { teachers: { some: { id: teacherId } } }],
            }
          : {}),
      ...
      include: {
        ...
        _count: {
          select: { students: true, dripConfigs: true },
        },
      },
...
export async function PATCH(request: Request) {
  ...
  try {
    const { id, name, classId, teacherIds, studentIds, endDate, isActive } = await request.json();

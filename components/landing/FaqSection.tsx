...
export default function FaqSection() {
  ...
  const FaqItem = ({ faq }: { faq: FaqItem; isLast: boolean }) => {
    ...
    return (
      <div className="group bg-white rounded-2xl shadow-sm hover:shadow-lg border border-gray-100 hover:border-blue-200 transition-all duration-300 overflow-hidden">
        <button
          onClick={() => toggleItem(faq.id)}
          className="w-full p-6 flex items-start justify-between text-left group-hover:bg-blue-50/30 transition-all duration-200"
        >
          <div className="flex-1 pr-6">
            <h3 className="text-gray-900 font-semibold text-base md:text-lg leading-relaxed mb-2 group-hover:text-blue-900 transition-colors duration-200">
              {faq.question}
            </h3>
          </div>
